...
def create_cost_input_DB_PC(version_str):
    ...
    with open(PC_path + 'JSON\\' + pcc.PC_COST_INPUT_DB_NAME + '_' + version_str + '.' + gbc.GB_JSON, 'w') as f:
        f.write(j)
    log('Finished PC cost input DB')

def upload_cost_input_DB_PC(version):
    connection =  os.environ[gbc.GB_SPECT_MOD_DATA_CONN_ENV]
    FQName = os.getcwd() + '\Tools\DefaultDataManager\PC\JSON\\' + pcc.PC_COST_INPUT_DB_NAME + '_' + version + '.' + gbc.GB_JSON
    container_name = gbc.GB_PC_CONTAINER
    GB_upload_file(connection, container_name, pcc.PC_COST_INPUT_DB_NAME + '_' + version + '.' + gbc.GB_JSON, FQName) 
