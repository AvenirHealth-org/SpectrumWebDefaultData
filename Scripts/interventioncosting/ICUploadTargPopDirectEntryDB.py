...
def create_targ_pop_direct_entry_DB_IC(version_str):
    ...
    for country_obj in country_list:    
        ...
        with open(IC_path + '\JSON\\' + icc.IC_TARG_POP_DIRECT_ENTRY_DB_DIR + '\\' + iso3 + '_' + version_str + '.' + gbc.GB_JSON, 'w') as f:
...
def upload_targ_pop_direct_entry_DB_IC(version):
    ...
    walk_path = os.getcwd() + '\Tools\DefaultDataManager\IC\JSON\\' + icc.IC_TARG_POP_DIRECT_ENTRY_DB_DIR + '\\'

    for subdir, dirs, files in os.walk(walk_path):
        for file in files:
            FQName = os.path.join(subdir, file)
            log(FQName)
            GB_upload_file(connection, gbc.GB_IC_CONTAINER, icc.IC_TARG_POP_DIRECT_ENTRY_DB_DIR + '\\' + file, FQName)
