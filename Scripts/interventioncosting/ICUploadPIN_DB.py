...
def create_PIN_DB_IC(version_str):
    ...
    os.makedirs(IC_path + '\JSON\\' + icc.IC_PIN_DB_DIR + '\\', exist_ok = True)
    for country_obj in country_list:    
        iso3 = country_obj[icdbc.IC_ISO_ALPHA_3_COUNTRY_CODE_KEY_PINDB]
        with open(IC_path + '\JSON\\' + icc.IC_PIN_DB_DIR + '\\' + iso3 + '_' + version_str + '.' + gbc.GB_JSON, 'w') as f:
...
def upload_PIN_DB_IC(version):
    ...
    walk_path = os.getcwd() + '\Tools\DefaultDataManager\IC\JSON\\' + icc.IC_PIN_DB_DIR + '\\'
